...
echo "# Trigger: [colmena] $SCRIPT_NAME" | tee -a $COLMENA_LOG_FILE
...
# Server IPs
ip a | grep "scope global" | awk '{print $2}' | sed -e 's@\/.*@@g'
# Gateways
ip -4 route | grep via | awk '{print $3}' | sort -u
ip -6 route | grep via | awk '{print $3}' | sort -u
# Last connected users
last | egrep -v "(reboot|wtmp|down|tty1|^$)" | awk '{print $3}' | sort -u
